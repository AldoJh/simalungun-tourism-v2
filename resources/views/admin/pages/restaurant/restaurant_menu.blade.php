    ...
    <div class="card-body pt-9 pb-0">
      <div class="d-flex flex-wrap flex-sm-nowrap mb-3">
        <div class="me-7 mb-4">
          <div class="position-relative">
              <img class="h-150px w-100 rounded" src="{{ Storage::url($restaurant->image) }}" alt="image">
          </div>
        </div>
          ...
          <div class="d-md-flex justify-content-between align-items-start mb-2">
            <div class="d-flex flex-column">
              <div class="d-flex align-items-center mb-2">
                <span class="text-gray-900 text-hover-primary fs-1 fw-bolder me-1">{{ $restaurant->name }}
                </span>
              </div>
              <div class="fs-6 fw-bold d-flex align-items-center mb-2">
                @if (count($restaurant->restaurantReview) > 0)
                  ...
                  @endfor
                  <span class="ms-2">{{ round($restaurant->restaurantReview->average('rating'), 1) }} ({{ count($restaurant->restaurantReview) }})</span>
                @else
                ...
                <div class="d-flex flex-wrap">
                    <div class="border border-gray-300 border-dashed rounded min-w-md-100px py-3 px-4 me-6 mb-3">
                      <div class="d-flex align-items-center">
                        <div class="fs-2 fw-bold counted" data-kt-countup="true" data-kt-countup-value="80" data-kt-initialized="1">{{ count($restaurant->restaurantViewer) }}</div>
                      </div>  
                      <div class="fw-semibold fs-6 text-gray-500">Viewer</div>
                    </div>
                    <div class="border border-gray-300 border-dashed rounded min-w-md-100px py-3 px-4 me-6 mb-3">
                      <div class="d-flex align-items-center">
                        <div class="fs-2 fw-bold counted" data-kt-countup="true" data-kt-countup-value="80" data-kt-initialized="1">{{ $restaurant->restaurantVisitor->sum('visitor') }}</div>
                      </div>  
      ...
      <div class="d-flex overflow-auto h-55px">
        <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bolder flex-nowrap">
          <li class="nav-item">
            <a href="{{ route('admin.restoran.restoran.pengunjung', $restaurant->id) }}" class="nav-link text-active-primary me-6">Pengunjung</a>
          </li>
          <li class="nav-item">
            <a href="{{ route('admin.restoran.restoran.review', $restaurant->id) }}" class="nav-link text-active-primary me-6">Review</a>
          </li>
          <li class="nav-item">
            <a href="{{ route('admin.restoran.restoran.galeri', $restaurant->id) }}" class="nav-link text-active-primary me-6">Galeri</a>
          </li>
          <li class="nav-item">
            <a href="{{ route('admin.restoran.restoran.menu', $restaurant->id) }}" class="nav-link text-active-primary me-6 active">Menu</a>
          </li>
          <li class="nav-item">
            <a href="{{ route('admin.restoran.restoran.admin', $restaurant->id) }}" class="nav-link text-active-primary me-6">Admin</a>
          </li>
        </ul>
      </div>
    </div>
      ...
      <div class="card-toolbar">
        ...
        <div class="modal fade" tabindex="-1" id="add">
          <div class="modal-dialog">
              <form method="POST" action="{{ route('admin.restoran.restoran.menu.store', $restaurant->id) }}" class="modal-content" enctype="multipart/form-data">
                @csrf
                  ...
                  <div class="modal-body">
                    ...
                    <div class="mb-3">
                      ...
                      <select name="type" class="form-control form-control-solid @error('type') is-invalid @enderror" required>
                        <option @if (old('type') == 'Makanan') selected  @endif value="Makanan">Makanan</option>
                        <option @if (old('type') == 'Minuman') selected  @endif value="Minuman">Minuman</option>
                      </select>
                      @error('type')
                        <div class="invalid-feedback">
                          {{ $message }}
                    ...
                    <div class="mb-3">
                      ...
                      <input type="number" name="price" class="form-control form-control-solid @error('price') is-invalid @enderror"  value="{{ old('price') }}" placeholder="Harga" required/>
                      @error('price')
                        <div class="invalid-feedback">
                          {{ $message }}
                        </div>
    ...
    <div class="card-body pt-0">
            ...
            @else
              @foreach ($menu as $item)     
                <tr>
                  ...
                  <td class="text-end">
                      <div class="fs-6 fw-bold">{{ $item->type }}</div>
                  </td>
                  <td class="text-end">
                    <div class="fs-6 fw-bold">Rp.{{ $item->price }}</div>
                  </td>
                  <td class="text-end">
                    ...
                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                      ...
                      <div class="menu-item px-3">
                        <a id="{{ route('admin.restoran.restoran.menu.destroy', ['id' => $restaurant->id, 'idMenu' => $item->id]) }}" class="menu-link px-3 btn-del">Hapus</a>
                      </div>
                    </div>
                  </td>
                </tr>
      ...
      <div class="d-flex flex-stack flex-wrap my-3">
        <div class="fs-6 fw-semibold text-gray-700">
            Showing {{ $menu->firstItem() }} to {{ $menu->lastItem() }} of {{ $menu->total() }}  records
        </div>
        <ul class="pagination">
            @if ($menu->onFirstPage())
            ...
            @else
                <li class="page-item previous">
                    <a href="{{ $menu->previousPageUrl() }}" class="page-link bg-light"><i class="previous"></i></a>
                </li>
            @endif
    
            @php
                // Menghitung halaman pertama dan terakhir yang akan ditampilkan
                $start = max($menu->currentPage() - 2, 1);
                $end = min($start + 4, $menu->lastPage());
            @endphp
            ...
            @foreach ($menu->getUrlRange($start, $end) as $page => $url)
                <li class="page-item{{ ($page == $menu->currentPage()) ? ' active' : '' }}">
                    <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                </li>
            @endforeach
    
            @if ($end < $menu->lastPage())
            ...
            @if ($menu->hasMorePages())
                <li class="page-item next">
                    <a href="{{ $menu->nextPageUrl() }}" class="page-link bg-light"><i class="next"></i></a>
                </li>
            @else
                <li class="page-item next">
...
@foreach ($menu as $item)     
  <div class="modal fade" tabindex="-1" id="edit{{ $item->id }}">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('admin.restoran.restoran.menu.update', ['id' => $restaurant->id, 'idMenu' => $item->id]) }}" class="modal-content" enctype="multipart/form-data">
          @csrf
            <div class="modal-header">
            ...
            <div class="modal-body">
              ...
              <div class="mb-3">
                <label for="exampleFormControlInput1" class="required form-label">Kategori</label>
                <select name="type" class="form-control form-control-solid @error('type') is-invalid @enderror" required>
                  <option @if ($item->type == 'Makanan') selected  @endif value="Makanan">Makanan</option>
                  <option @if ($item->type == 'Minuman') selected  @endif value="Minuman">Minuman</option>
                </select>
                @error('type')
                  <div class="invalid-feedback">
                    {{ $message }}
              ...
              <div class="mb-3">
                <label for="exampleFormControlInput1" class="required form-label">Price</label>
                <input type="number" name="price" class="form-control form-control-solid @error('price') is-invalid @enderror"  value="{{ old('price') ?? $item->price }}" placeholder="Harga" required/>
                @error('price')
                  <div class="invalid-feedback">
                    {{ $message }}
                  </div>
