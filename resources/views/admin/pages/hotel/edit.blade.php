...
@section('content')
  ...
  <form action="{{ route('admin.hotel.hotel.update', $hotel->id) }}" enctype="multipart/form-data" method="post" class="row g-5 g-xl-8">
    @csrf
        ...
        <div class="card card-flush">
          ...
          <div class="card-body text-center pt-0">
            <div class="image-input image-input-empty" data-kt-image-input="true">
              <div class="image-input-wrapper w-200px h-150px" style="background-image: url('{{ Storage::url($hotel->image) }}')"></div>
        ...
        <div class="card card-flush">
          <div class="card-body pt-0">
            <div class="mb-3 mt-9">
              ...
              <input type="text" class="form-control form-control-solid @error('owner') is-invalid @enderror" name="owner" placeholder="Nama Pemilik" value="{{ old('owner') ?? $hotel->owner }}" required/>
              @error('owner')
                <div class="invalid-feedback">
                  {{ $message }}
                </div>
            ...
            <div class="mb-3">
              ...
              <textarea class="form-control form-control-solid @error('meta') is-invalid @enderror" rows="5" name="meta" placeholder="Tulis deskripsi singkat..." required>{{ old('meta') ?? $hotel->excerpt }}</textarea>
              @error('meta')
                <div class="invalid-feedback">
                  {{ $message }}
                </div>
          ...
          <div class="card-body pt-0">
            <div class="row">
              <div class="col-8 mb-3 mt-9">
                ...
                <input type="text" name="name" class="form-control form-control-solid @error('name') is-invalid @enderror" value="{{ old('name') ?? $hotel->name }}" placeholder="Nama Festival" required/>
                @error('name')
                  <div class="invalid-feedback">
                    {{ $message }}
                  </div>
              ...
              <div class="col-4 mb-3 mt-9">
                ...
                <select name="category" class="form-control form-select form-control-solid @error('category') is-invalid @enderror" required>
                  ...
                  @foreach ($category as $item)
                    <option @if ($item->id == $hotel->hotelCategory->id) selected  @endif value="{{ $hotel->hotelCategory->id }}">{{ $item->name }}</option>
                  @endforeach
                </select>
                @error('category')
                <div class="invalid-feedback">
            ...
            <div class="mb-5">
              ...
              <input type="text" name="phone" class="form-control form-control-solid @error('phone') is-invalid @enderror" value="{{ old('phone')?? $hotel->phone }}" placeholder="No. Telepon" required/>
              @error('phone')
                <div class="invalid-feedback">
                  {{ $message }}
                </div>
            ...
            <div class="row">
              <div class="col mb-5">
                ...
                <select name="district" id="kecamatan" class="form-control form-select form-control-solid @error('district') is-invalid @enderror" required>
                  ...
                  @foreach ($district as $item)
                    <option @if ($hotel->district_id ==  $item->id) selected  @endif data-id="{{ $item->code }}" value="{{ $item->id }}">{{ $item->name }}</option>
                  @endforeach
                </select>
                @error('district')
                  <div class="invalid-feedback">
              ...
              <div class="col mb-5">
                ...
                <select name="village" id="kelurahan" class="form-control form-select form-control-solid @error('village') is-invalid @enderror" required>
                  ...
                  <option  data-id="{{ $item->code }}" value="{{ $hotel->village_id }}">{{ $hotel->village->name }}</option>
                </select>
                @error('village')
                    <div class="invalid-feedback">
                        {{ $message }}
            ...
            <div class="row">
              <div class="col mb-5">
                ...
                <input type="number" name="lat" class="form-control form-control-solid @error('lat') is-invalid @enderror" value="{{ old('lat') ?? $hotel->latitude }}" placeholder="Latitude" required/>
                @error('lat')
                  <div class="invalid-feedback">
                    {{ $message }}
                  </div>
              ...
              <div class="col mb-5">
                ...
                <input type="number" name="long" class="form-control form-control-solid @error('long') is-invalid @enderror" value="{{ old('long') ?? $hotel->longitude }}" placeholder="Longitude" required/>
                @error('long')
                  <div class="invalid-feedback">
                    {{ $message }}
                  </div>
            ...
            <div class="mb-5">
              ...
              <textarea name="address" class="form-control form-control-solid @error('address') is-invalid @enderror" placeholder="Alamat lengkap" rows="3" required>{{ old('address') ?? $hotel->address }}</textarea>
              @error('address')
                <div class="invalid-feedback">
                  {{ $message }}
                </div>
            ...
            <div class="mb-5">
              ...
              <select class="form-select form-select-solid" data-control="select2" data-close-on-select="false" data-placeholder="Select an option" data-allow-clear="true" multiple="multiple" name="facility[]">
                @foreach ($facility as $item)
                  <option value="{{ $item->id }}" @if(in_array($item->id, $hotel->hotelFacility->pluck('facility_id')->toArray())) selected @endif>{{ $item->name }}</option>
              @endforeach
              </select>
              @error('facility')
                  <div class="invalid-feedback">
            ...
            <div class="mb-5">
              ...
              <input type="number" name="room" class="form-control form-control-solid @error('room') is-invalid @enderror" value="{{ old('room') ?? $hotel->room }}" placeholder="Rp.0" required/>
              @error('room')
                <div class="invalid-feedback">
                  {{ $message }}
                </div>
            ...
            <div class="row">
              <div class="col mb-5">
                ...
                <input type="number" name="min" class="form-control form-control-solid @error('min') is-invalid @enderror" value="{{ old('min') ?? $hotel->min_price }}" placeholder="Rp.0" required/>
                @error('min')
                  <div class="invalid-feedback">
                    {{ $message }}
                  </div>
              ...
              <div class="col mb-5">
                ...
                <input type="number" name="max" class="form-control form-control-solid @error('max') is-invalid @enderror" value="{{ old('max')?? $hotel->max_price }}" placeholder="Rp.0" required/>
                @error('max')
                  <div class="invalid-feedback">
                    {{ $message }}
                  </div>
