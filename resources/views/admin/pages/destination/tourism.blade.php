        ...
        <div class="card-body pt-6">
            <ul class="nav nav-pills nav-pills-custom mb-3" role="tablist">
                    <li class="nav-item mb-3 me-3 me-lg-6" role="presentation">
                        <a href="{{ route('admin.admin-destinasi.wisata') }}" class="nav-link btn btn-outline btn-flex btn-color-muted btn-active-color-primary flex-column overflow-hidden w-80px h-85px pt-5 pb-2 active" >
                        ...
                        @foreach ($tourism as $item)     
                            ...
                            <td>
                                ...
                                <a href="#" class="symbol symbol-50px">
                                    @if(Storage::disk('public')->exists($item->tourism->image))
                                        <span class="symbol-label" style="background-image:url('{{ Storage::url($item->tourism->image) }}');"></span>
                                    @else
                                    <span class="symbol-label" style="background-image:url('{{ asset('front-assets/no-image.png') }}');"></span>
                                    @endif
                                </a>
                                <div class="ms-5">
                                    <a href="{{ route('wisata.show', $item->tourism->slug) }}" target="_blank" class="text-gray-800 text-hover-primary fs-5 fw-bold mb-1">
                                    {{ $item->tourism->name }}
                                    @if ($item->tourism->is_recomended) <span class="badge bg-light-success text-success">Recomended</span> @endif
                                    </a>
                                    <div class="text-muted fs-7 fw-bold">{{ $item->tourism->address }}</div>                    
                                </div>
                                </div>
                            </td>
                            <td>
                                <div>
                                <div class="fs-6 fw-bold">{{ $item->tourism->tourismCategory->name }}</div>
                                </div>
                            </td>
                            <td>
                                <div class="text-center">
                                <div class="fs-6 fw-bold d-flex align-items-center">
                                    @if (count($item->tourism->tourismReview) > 0)
                                    @for($i = 0; $i < round($item->tourism->tourismReview->average('rating'), 0); $i++)
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star-fill text-warning" viewBox="0 0 16 16">
                                        <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
                                        </svg>
                                    @endfor
                                    <span class="ms-2">{{ round($item->tourism->tourismReview->average('rating'), 1) }} ({{ count($item->tourism->tourismReview) }})</span>
                                    @else
                            ...
                            <td>
                                <div class="text-center">
                                <div class="fs-6 fw-bold">{{ $item->tourism->tourismVisitor->sum('visitor') }}</div>
                                </div>
                            </td>
                            <td>
                                <div class="text-center">
                                <div class="fs-6 fw-bold">{{ count($item->tourism->tourismViewer) }}</div>
                                </div>
                            </td>
                            <td class="text-end">
