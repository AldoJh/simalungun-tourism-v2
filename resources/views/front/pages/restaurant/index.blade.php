...
@section('seo')
  ...
  <meta property="og:url" content="{{ route('restoran') }}" />
  <meta property="og:title" content="Simalungun | {{$title ?? ''}}" />
  <meta property="og:type" content="article" />
  <meta property="og:image" content="{{ asset('front-assets/meta/restoran.png') }}" />
  <meta
  ...
  <meta
    ...
    content="{!! \App\Models\Setting::webBase()->description !!}"
  />
  <meta name="twitter:image" content="{{ asset('front-assets/meta/restoran.png') }}" />
  ...
  <!-- Canonical Tag -->
  <link rel="canonical" href="{{ route('restoran') }}" />
@endsection
    ...
    <div class="row pt-10">
      <div class="col-12 d-lg-flex justify-between">
        ...
        <div class="dropdown -base -price js-dropdown js-form-dd" data-main-value="">
          ...
          <form method="GET" {{ route('restoran') }} class="dropdown__menu px-30 py-30 shadow-1 border-1 js-">
            <h5 class="text-18 fw-500">Cari Restourant</h5>
            <input type="hidden" name="page" value="{{ request('page', 1) }}">
            <div class="pt-20">
              <div class="d-flex flex-column y-gap-15">
                <input class="form-control me-2" type="search" placeholder="Cari Restaurant" name="q" value="{{ request('q') }}" aria-label="Search">
              </div>
            </div>
  ...
  <div class="container">
    ...
    <div class="d-flex justify-center flex-column mt-60">
      <div class="pagination justify-center">
          ...
          @else
              <a href="{{ $restaurant->previousPageUrl() }}" class="pagination__button button -accent-1 mr-15 -prev">
                  <i class="icon-arrow-left text-15"></i>
              </a>
          @endif
          ...
          <div class="pagination__count">
              @php
                  ...
                  $end = min($start + 2, $restaurant->lastPage());
              @endphp
  
              @if ($start > 1)
                  <a href="{{ $restaurant->url(1) }}" class="{{ ($restaurant->currentPage() == 1) ? ' is-active' : '' }}">1</a>
                  <span>...</span>
              @endif
  
              @foreach ($restaurant->getUrlRange($start, $end) as $page => $url)
                  <a href="{{ $url }}" class="{{ ($page == $restaurant->currentPage()) ? ' is-active' : '' }}">{{ $page }}</a>
              @endforeach
  
              @if ($end < $restaurant->lastPage())
                  <span>...</span>
                  <a href="{{ $restaurant->url($restaurant->lastPage()) }}" class="{{ ($restaurant->currentPage() == $restaurant->lastPage()) ? ' is-active' : '' }}">{{ $restaurant->lastPage() }}</a>
              @endif
          </div>
  
          @if ($restaurant->hasMorePages())
              <a href="{{ $restaurant->nextPageUrl() }}" class="pagination__button button -accent-1 ml-15 -next">
      ...
      <div class="text-14 text-center mt-20">Showing results {{ $restaurant->firstItem() }}-{{ $restaurant->lastItem() }} of {{ $restaurant->total() }}</div>
    </div>
  </div>
</section>
